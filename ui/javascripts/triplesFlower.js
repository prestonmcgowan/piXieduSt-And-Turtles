...
CodeFlower.prototype.update = function(json) {
  ...
  this.node.enter()
      .append('svg:circle')
       ...
       .call(top.force.drag)
       .on("click", top.click.bind(top))
       .on("mouseover", top.mouseover.bind(top))
       .on("mouseout", top.mouseout.bind(top));
...
CodeFlower.prototype.flatten = function(root) {
  ...
  var returnMe = [];
  for (var k in nodes) {
      returnMe.push(nodes[k]);
  }
  returnMe.size = nodes.size;
  return returnMe;
};
*/
...
CodeFlower.prototype.flatten = function(root) {
  ...
  function recurse(node) {
    ...
    if (!node.id) 
        node.id = ++i;
    console.log("found:" + ids.indexOf(node.id));
